...
if (associated(profile%data_t)) then
...
s_avail=.false.
if (profile%nvar == 2 .and. associated(profile%data_s)) then
...
days_since = days + secs/86400.
 ...
 if (s_avail) call mpp_write(unit,data_s_field,data_s,station)
...
!   call mpp_write(unit,profile_flag_s_field,profile_flag,station)
! endif
 ...
 if (s_avail) call mpp_write(unit,salt_err_field,profile%salt_err,station)
 nlinks = 0
