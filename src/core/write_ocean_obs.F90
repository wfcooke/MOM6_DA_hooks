...
type(fieldtype), save :: lon_field, lat_field, time_field, data_t_field, data_s_field, &
        ...
        fcst_t_field, fcst_s_field, ana_t_field, ana_s_field, &
...
function open_profile_file(name, nvar, grid_lon, grid_lat,thread,fset)
  ...
  call mpp_write_meta(unit,profile_flag_field,(/station_axis/),&
          'profile_flag','none','profile_flag',min=0.0,max=10.0,missing=missing_value)
  ...
  call mpp_write_meta(unit,data_t_field,(/depth_axis,station_axis/),&
          'temp',trim(units),'in-situ temperature',min=-10.0,max=50.0,missing=missing_value)
 
  units='g/kg'
  if (nvar_out .eq. 2) call mpp_write_meta(unit,data_s_field,(/depth_axis,station_axis/),&
          'salt',trim(units),'salinity',min=0.0,max=50.0,missing=missing_value)
  
  units='degrees_C'
  call mpp_write_meta(unit,fcst_t_field,(/depth_axis,station_axis/),&
          'temp_prior',trim(units),'first-guess in-situ temperature',min=-10.0,max=50.0,missing=missing_value)
 
  units='g/kg'
  if (nvar_out .eq. 2) call mpp_write_meta(unit,fcst_s_field,(/depth_axis,station_axis/),&
          'salt_prior',trim(units),'first-guess salinity',min=0.0,max=50.0,missing=missing_value)
  
  units='degrees_C'
  call mpp_write_meta(unit,ana_t_field,(/depth_axis,station_axis/),&
          'temp_posterior',trim(units),'analysis in-situ temperature',min=-10.0,max=50.0,missing=missing_value)
 
  units='g/kg'
  if (nvar_out .eq. 2) call mpp_write_meta(unit,ana_s_field,(/depth_axis,station_axis/),&
          'salt_posterior',trim(units),'analysis salinity',min=0.0,max=50.0,missing=missing_value)
...
subroutine write_profile(unit,profile)
  ...
  real, dimension(max_levels_file) :: fcst_t, fcst_s, ana_t, ana_s
  ...
  nvar = profile%variable
  levels = min(profile%levels,max_levels_file)
  data_t=missing_value;data_s=missing_value;depth=missing_value
  fcst_t=missing_value;fcst_s=missing_value
  ana_t=missing_value;ana_s=missing_value
  flag_t=missing_value;flag_s=missing_value
  if (nvar == 1) then
      data_t(1:levels)=profile%data(1:levels)
      if(f_avail) fcst_t(1:levels)=profile%forecast(1:levels)
      if(a_avail) ana_t(1:levels)=profile%analysis(1:levels)
  !    flag_t(1:levels)=profile%flag(1:levels)
  elseif (nvar == 2) then 
      data_s(1:levels)=profile%data(1:levels)
      if(f_avail) fcst_s(1:levels)=profile%forecast(1:levels)
      if(a_avail) ana_s(1:levels)=profile%analysis(1:levels)
  !    flag_s(1:levels)=profile%flag(1:levels)
  endif
  ...
  if (nvar == 1) then
    ...
    if(f_avail) call mpp_write(unit,fcst_t_field,fcst_t,station)
    if(a_avail) call mpp_write(unit,ana_t_field,ana_t,station)
  elseif (nvar == 2) then
    call mpp_write(unit,data_s_field,data_s,station)
    if(f_avail) call mpp_write(unit,fcst_s_field,fcst_s,station)
    if(a_avail) call mpp_write(unit,ana_s_field,ana_s,station)
  endif
 ...
 ! endif
  ...
  do i = 1, nlinks
     ...
     if (nvar == 1) then
       data_t(1:levels)=profile%data((max_levels_file*i)+1:(max_levels_file*i)+levels)
       if(f_avail) fcst_t(1:levels)=profile%forecast((max_levels_file*i)+1:(max_levels_file*i)+levels)
       if(a_avail) ana_t(1:levels)=profile%analysis((max_levels_file*i)+1:(max_levels_file*i)+levels)
  !     flag_t(1:levels)=profile%flag((max_levels_file*i)+1:(max_levels_file*i)+levels)
     elseif (nvar == 2) then
       data_s(1:levels)=profile%data((max_levels_file*i)+1:(max_levels_file*i)+levels)
       if(f_avail) fcst_s(1:levels)=profile%forecast((max_levels_file*i)+1:(max_levels_file*i)+levels)
       if(a_avail) ana_s(1:levels)=profile%analysis((max_levels_file*i)+1:(max_levels_file*i)+levels)
  !     flag_s(1:levels)=profile%flag((max_levels_file*i)+1:(max_levels_file*i)+levels)
     endif
